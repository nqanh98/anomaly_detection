...
class HotspotDetectors():
    def __init__(self):
        ...
        # -- temperature dependent correction (not used in current version) --
    ...
    def fit(self, thermal_data, module_labels):
        ...
        for c in tqdm(range(0,max(module_labels)+1)):
            ...
            #self.lof[c].offset_ = self.alpha_lof - self.beta_lof * self.correction_term[c]
            # -- Isolation Forest --
            ...
            #self.isof[c].offset_ = self.alpha_isof - self.beta_isof * self.correction_term[c]
